...
class MultiWysiwygPreview(QWidget):
    ...
    def paintEvent(self, _event) -> None:
        ...
        shadow.setAlpha(45)
        painter.fillRect(paper.translated(3, 3), shadow)
        painter.fillRect(paper, Qt.GlobalColor.white)
        transform = MmTransform(plan, paper)
        painter.setPen(QPen(self.palette().color(QPalette.ColorRole.Mid), 1, Qt.PenStyle.DashLine))
        painter.drawRect(transform.rect_to_target(plan.printable_rect))
